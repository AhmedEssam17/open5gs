...
service_area_restriction_t* instantiate_service_area_restriction(int include_optional);
...
service_area_restriction_t* instantiate_service_area_restriction(int include_optional) {
  ...
  if (include_optional) {
    service_area_restriction = service_area_restriction_create(
      nchf_convergedcharging_service_area_restriction__ALLOWED_AREAS,
      list_createList(),
      0,
      0
    );
...
void test_service_area_restriction(int include_optional) {
    service_area_restriction_t* service_area_restriction_1 = instantiate_service_area_restriction(include_optional);

	cJSON* jsonservice_area_restriction_1 = service_area_restriction_convertToJSON(service_area_restriction_1);
	printf("service_area_restriction :\n%s\n", cJSON_Print(jsonservice_area_restriction_1));
	service_area_restriction_t* service_area_restriction_2 = service_area_restriction_parseFromJSON(jsonservice_area_restriction_1);
	cJSON* jsonservice_area_restriction_2 = service_area_restriction_convertToJSON(service_area_restriction_2);
