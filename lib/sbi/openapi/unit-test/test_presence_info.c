...
presence_info_t* instantiate_presence_info(int include_optional) {
  ...
  if (include_optional) {
    presence_info = presence_info_create(
      "0",
      "0",
      nchf_convergedcharging_presence_info__IN_AREA,
      list_createList(),
      list_createList(),
      list_createList(),
      list_createList(),
...
void test_presence_info(int include_optional) {
    presence_info_t* presence_info_1 = instantiate_presence_info(include_optional);

	cJSON* jsonpresence_info_1 = presence_info_convertToJSON(presence_info_1);
	printf("presence_info :\n%s\n", cJSON_Print(jsonpresence_info_1));
	presence_info_t* presence_info_2 = presence_info_parseFromJSON(jsonpresence_info_1);
	cJSON* jsonpresence_info_2 = presence_info_convertToJSON(presence_info_2);
	printf("repeating presence_info:\n%s\n", cJSON_Print(jsonpresence_info_2));
}
